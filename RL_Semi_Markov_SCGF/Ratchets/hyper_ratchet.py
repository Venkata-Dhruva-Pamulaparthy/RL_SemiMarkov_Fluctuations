...
def main():
    ...
    for bias in biases:
        ...
        for epoch in range(epochs):
		    ...
		    # take the action
                ...
                pdf_FF=  0.5*((p1*torch.exp(-p1*tau)) +(p2*torch.exp(-tau*p2))) 
                phi_F = (0.5)*((torch.exp(-p1*tau))+(torch.exp(-p2*tau)))
                pdf_FF1=  0.5*((p1*torch.exp(-p1*tau1)) +(p2*torch.exp(-tau1*p2))) 
                phi_F1 = (0.5)*((torch.exp(-p1*tau1))+(torch.exp(-p2*tau1)))                
    ...
    out=0
    np.save(f"hyper_ratchet.npy",avg_save)
