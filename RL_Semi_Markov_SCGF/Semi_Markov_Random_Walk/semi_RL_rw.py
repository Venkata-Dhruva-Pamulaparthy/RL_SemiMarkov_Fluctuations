...
class Actor(nn.Module):
      ...
      def __init__(self):
        ...
        self.action_head_mov = nn.Linear(64,2)
        self.log_probs=[]
      ...
      def forward(self, x):
        ...
        move_params= (self.action_head_mov(x1))
...
def main():
    ...
    for bias in biases:
        ...
        reset_nets()
        x_state = torch.zeros((batch_size,1),dtype=torch.float).to(device)
        wait_state=  torch.rand((batch_size,1),dtype=torch.float).to(device) 
        next_state_x = torch.zeros((batch_size,1),dtype=torch.float).to(device)
        ...
        t=[]
        time1=time.time()
        a=2*torch.ones((batch_size,1),dtype=torch.float).to(device)
        for epoch in range(epochs):
		    ...
		    # take the action
                ...
                #print(alpha,beta,a)
                comp1=(tau*torch.exp(-tau*alpha) * alpha**2) *torch.special.gammaincc(a, beta*tau)
                comp2 = (tau*torch.exp(-tau*beta) * beta**2) *torch.special.gammaincc(a,alpha*tau)
                comp11= (tau1*torch.exp(-tau1*alpha) * alpha**2) * torch.special.gammaincc(a,beta*tau1)
                comp21 =  (tau1*torch.exp(-tau1*beta) * beta**2) *torch.special.gammaincc(a,alpha*tau1)
             
                log_org_waiting= torch.log(comp11+comp21) 
                log_org_prob[action==0]= comp1[action==0]/(comp1[action==0] + comp2[action==0])
                log_org_prob[action==1]= comp2[action==1]/(comp1[action==1] + comp2[action==1]) 
                bias_tensor[action==0]=bias
                bias_tensor[action==1]=-bias
                ...
                pol1.append(policy_loss1.item())
                pol2.append(policy_loss2.item())
                crit.append(critic_loss.item())
               ...
               # writer.add_scalar(f"Loss/reward_{bias}",reward.mean(),i)
                ...
                ret.append(average_reward.mean())
                #for x
                #average_current[==0] +=
    ...
    out=0
    np.save("semi_rw_metric.npy",avg_save)
    np.save("policy_loss1.npy",pol1)
    np.save("policy_loss2.npy",pol2)
    np.save("crit.npy",crit)
    np.save("ret.npy",np.array(RET,dtype=object),allow_pickle=True)
    np.save("time.npy",np.array(TIME,dtype=object),allow_pickle=True)
